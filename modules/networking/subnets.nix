    ...
    nixos =
      { config, lib, ... }:
      {
        options.networking = {
          ...
          subnetExtraHosts = lib.mkOption {
            type = lib.types.str;
            readOnly = true;
            description = "Hosts file entries generated from subnets' namedAddresses";
          };
        };

        config = {
          networking.subnetExtraHosts =
            let
              allLines = lib.concatLists (
                lib.mapAttrsToList (
                  subnetName: subnet:
                  lib.mapAttrsToList (name: ip: "${ip} ${name}.${subnetName}") subnet.namedAddresses
                ) config.networking.subnets
              );
            in
